...
class ModuleImportWp extends Module
{
    ...
    protected function imForm(int $step, string $legend, ?string $submit_value = null)
    {
        ...
        '<div>' . dcCore::app()->nonce->form()->render() .
    ...
    protected function importUsers(): void
    {
        ...
        try {
            ...
            while ($rs->fetch()) {
                ...
                if (!dcCore::app()->users->has($user_login)) {
    ...
    protected function importPost($rs, $db): void
    {
        ...
        $cur->post_content_xhtml = dcCore::app()->formater->callLegacy($this->vars['post_formater'], $cur->post_content);
        $cur->post_excerpt_xhtml = dcCore::app()->formater->callLegacy($this->vars['post_formater'], $cur->post_excerpt);
    ...
    protected function importComments(string $post_id, int $new_post_id, $db): void
    {
        ...
        while ($rs->fetch()) {
            ...
            $cur->comment_content   = dcCore::app()->formater->callLegacy($this->vars['comment_formater'], Text::cleanStr($rs->comment_content));
