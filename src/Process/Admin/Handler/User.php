...
class User extends AbstractPage
{
    ...
    protected function getPagePrepend(): ?bool
    {
        ...
        if (isset($_POST['user_name'])) {
            try {
                ...
                $this->container->set('user_id', $_POST['user_id']);
                $this->container->set('user_super', !empty($_POST['user_super']));
                $this->container->set('user_name', Html::escapeHTML($_POST['user_name']));
                $this->container->set('user_firstname', Html::escapeHTML($_POST['user_firstname']));
                $this->container->set('user_displayname', Html::escapeHTML($_POST['user_displayname']));
                $this->container->set('user_email', Html::escapeHTML($_POST['user_email']));
                $this->container->set('user_url', Html::escapeHTML($_POST['user_url']));
                $this->container->set('user_lang', Html::escapeHTML($_POST['user_lang']));
                $this->container->set('user_tz', Html::escapeHTML($_POST['user_tz']));
                $this->container->set('user_post_status', Html::escapeHTML($_POST['user_post_status']));

                if ($this->container->get('user_id') == dotclear()->user()->userID() && dotclear()->user()->isSuperAdmin()) {
                    // force super_user to true if current user
                    $this->container->set('user_super', true);
                }
                if (dotclear()->user()->allowPassChange()) {
                    $this->container->set('user_change_pwd', !empty($_POST['user_change_pwd']) ? 1 : 0);
                }
                ...
                if (!empty($_POST['new_pwd'])) {
                    ...
                    $this->container->set('user_pwd', $_POST['new_pwd']);
                }
    ...
    protected function getPageContent(): void
    {
        ...
        Form::combo('user_lang', dotclear()->combo()->getAdminLangsCombo(), $this->container->get('user_lang'), 'l10n') .
        '</p>' .

        '<p><label for="user_tz">' . __('Timezone:') . '</label> ' .
        Form::combo('user_tz', Dt::getZones(true, true), $this->container->get('user_tz')) .
        '</p>' .
        ...
        Form::combo('user_post_status', dotclear()->combo()->getPostStatusesCombo(), $this->container->get('user_post_status')) .
        '</p>' .
        ...
        ('' != $this->container->get('user_id') ? '' : ' <input type="submit" name="saveplus" value="' . __('Save and create another') . '" />') .
        ('' != $this->container->get('user_id') ? Form::hidden('id', $this->container->get('user_id')) : '') .
        ...
        if (!$this->container->get('user_super')) {
            ...
            dotclear()->adminurl()->getHiddenFormFields('admin.user.actions', [
                ...
                'action'  => 'blogs',
                'users[]' => $this->container->get('user_id'),
            ], true) . '</p>' .
                '</form>';

            $permissions = dotclear()->users()->getUserPermissions($this->container->get('user_id'));
            ...
            echo '<p>' . sprintf(__('%s is super admin (all rights on all blogs).'), '<strong>' . $this->container->get('user_id') . '</strong>') . '</p>';
        }
        echo '</div>';
