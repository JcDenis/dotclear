...
class Home extends AbstractPage
{
    ...
    protected function getPagePrepend(): ?bool
    {
        ...
        if (!App::core()->user()->preferences('dashboard')->hasLocalPreference('doclinks')) {
            if (!App::core()->user()->preferences('dashboard')->hasGlobalPreference('doclinks')) {
                App::core()->user()->preferences('dashboard')->putPreference('doclinks', true, 'boolean', '', null, true);
            }
            App::core()->user()->preferences('dashboard')->putPreference('doclinks', true, 'boolean');
        }
        if (!App::core()->user()->preferences('dashboard')->hasLocalPreference('dcnews')) {
            if (!App::core()->user()->preferences('dashboard')->hasGlobalPreference('dcnews')) {
                App::core()->user()->preferences('dashboard')->putPreference('dcnews', true, 'boolean', '', null, true);
            }
            App::core()->user()->preferences('dashboard')->putPreference('dcnews', true, 'boolean');
        }
        if (!App::core()->user()->preferences('dashboard')->hasLocalPreference('quickentry')) {
            if (!App::core()->user()->preferences('dashboard')->hasGlobalPreference('quickentry')) {
                App::core()->user()->preferences('dashboard')->putPreference('quickentry', false, 'boolean', '', null, true);
            }
            App::core()->user()->preferences('dashboard')->putPreference('quickentry', false, 'boolean');
        }
        if (!App::core()->user()->preferences('dashboard')->hasLocalPreference('nodcupdate')) {
            if (!App::core()->user()->preferences('dashboard')->hasGlobalPreference('nodcupdate')) {
                App::core()->user()->preferences('dashboard')->putPreference('nodcupdate', false, 'boolean', '', null, true);
            }
            App::core()->user()->preferences('dashboard')->putPreference('nodcupdate', false, 'boolean');
        }
        ...
        if (!App::core()->user()->preferences('toggles')->hasLocalPreference('unfolded_sections')) {
            App::core()->user()->preferences('toggles')->putPreference('unfolded_sections', '', 'string', 'Folded sections in admin', null, true);
        }

        // Editor stuff
        $admin_post_behavior = '';
        if (App::core()->user()->preferences('dashboard')->getPreference('quickentry')) {
    ...
    protected function getPageContent(): void
    {
        ...
        // Dashboard icons
        if (!App::core()->user()->preferences('dashboard')->getPreference('nofavicons')) {
            $icons = '';
        ...
        // Documentation links
        if (App::core()->user()->preferences('dashboard')->getPreference('doclinks')) {
    ...
    {
        $order = App::core()->user()->preferences('dashboard')->getPreference($part);
