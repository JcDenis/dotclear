...
class ThemeList extends PluginList
{
    public function displayData(array $cols = ['name', 'version', 'description'], array $actions = [], bool $nav_limit = false): static
    {
        ...
        foreach ($modules as $id => $module) {
            ...
            $current = App::core()->blog()->settings('system')->getSetting('theme') == $id && $this->modules()->hasModule($id);
    ...
    protected function getActions(string $id, ModuleDefine $module, array $actions): array
    {
        ...
        if (App::core()->blog()->settings('system')->getSetting('theme') != $id) {
    ...
    public function doActions(): void
    {
        ...
        if (!GPC::post()->empty('select')) {
            ...
            if (count(GPC::post()->array('select'))) {
                ...
                App::core()->blog()->settings('system')->putSetting('theme', $id);
