...
class Upgrade
{
    ...
    public function growUp(?string $version): bool
    {
        ...
        if (version_compare($version, '2.2-alpha1-r3043', '<')) {
            ...
            if (dotclear()->plugins->moduleExists('metadata')) {
                dotclear()->plugins->deleteModule('metadata');
            }
        ...
        if (version_compare($version, '2.7', '<=')) {
            ...
            $strReqCount = 'SELECT count(1) FROM ' . dotclear()->prefix . 'setting';
            $strReqCount .= ' WHERE setting_id = \'%s\'';
            $strReqCount .= ' AND setting_ns = \'system\'';
            $strReqCount .= ' AND blog_id IS NULL';

            $strReqSelect = 'SELECT setting_value FROM ' . dotclear()->prefix . 'setting';
            $strReqSelect .= ' WHERE setting_id = \'%s\'';
            $strReqSelect .= ' AND setting_ns = \'system\'';
            $strReqSelect .= ' AND blog_id IS NULL';
        ...
        if (version_compare($version, '2.16.9', '<')) {
            ...
            $strReq = 'UPDATE ' . dotclear()->prefix . 'pref ' .
    ...
    {
        $strReqSelect = 'SELECT setting_id,blog_id,setting_ns,setting_type,setting_value FROM ' . dotclear()->prefix . 'setting ' .
            "WHERE setting_id = '%s' " .
            "AND setting_ns = '%s' " .
    ...
    {
        $strReqSelect = 'SELECT pref_id,user_id,pref_ws,pref_type,pref_value FROM ' . dotclear()->prefix . 'pref ' .
            "WHERE pref_id = '%s' " .
            "AND pref_ws = '%s' " .
