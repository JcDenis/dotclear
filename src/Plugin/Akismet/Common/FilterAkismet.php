...
class FilterAkismet extends Spamfilter
{
    ...
    {
        if (!App::core()->blog()->settings('akismet')->getSetting('ak_key')) {
            return false;
        }

        return new Akismet(App::core()->blog()->url, App::core()->blog()->settings('akismet')->getSetting('ak_key'));
    }
    ...
    {
        $ak_key      = App::core()->blog()->settings('akismet')->getSetting('ak_key');
        $ak_verified = null;
        ...
        if (GPC::post()->isset('ak_key')) {
            try {
                ...
                App::core()->blog()->settings('akismet')->putSetting('ak_key', $ak_key, 'string');
        ...
        if (App::core()->blog()->settings('akismet')->getSetting('ak_key')) {
            try {
                $ak          = new Akismet(App::core()->blog()->url, App::core()->blog()->settings('akismet')->getSetting('ak_key'));
