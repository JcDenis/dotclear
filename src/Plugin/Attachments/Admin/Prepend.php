...
class Prepend extends AbstractPrepend
{
    ...
    public static function loadModule(): void
    {
        ...
        dcCore()->behaviors->add('adminPostAfterForm', [__CLASS__, 'behaviorAdminPostAfterForm']);
        dcCore()->behaviors->add('adminPostHeaders', [__CLASS__, 'behaviorAdminPostHeaders']);
        dcCore()->behaviors->add('adminPageFormItems', [__CLASS__, 'behaviorAdminPostFormItems']);
        dcCore()->behaviors->add('adminPageAfterForm', [__CLASS__, 'behaviorAdminPostAfterForm']);
        dcCore()->behaviors->add('adminPageHeaders', [__CLASS__, 'behaviorAdminPostHeaders']);
        dcCore()->behaviors->add('adminPageHelpBlock', [__CLASS__, 'behaviorAadminPageHelpBlock']);
    }
    ...
    {
        if ($post !== null) {
            $post_media = dcCore()->mediaInstance()->getPostMedia($post->post_id, null, 'attachment');
            ...
            foreach ($post_media as $f) {
                ...
                '<a class="media-icon" href="' . dcCore()->adminurl->get('admin.media.item', ['id' => $f->media_id]) . '">' .
                '<img src="' . $f->media_icon . '" alt="" title="' . $f->basename . '" /></a>' .
                '<ul>' .
                '<li><a class="media-link" href="' . dcCore()->adminurl->get('admin.media.item', ['id' => $f->media_id]) . '" ' .
                ...
                'href="' . dcCore()->adminurl->get('admin.post.media', [
            ...
            $item .= '<p class="s-attachments"><a class="button" href="' . dcCore()->adminurl->get('admin.media', ['post_id' => $post->post_id, 'link_type' => 'attachment']) . '">' .
    ...
    public static function behaviorAdminPostAfterForm(?Record $post)
    {
        if ($post !== null) {
            echo
            '<form action="' . dcCore()->adminurl->get('admin.post.media') . '" id="attachment-remove-hide" method="post">' .
            '<div>' . form::hidden(['post_id'], $post->post_id) .
            form::hidden(['media_id'], '') .
            form::hidden(['link_type'], 'attachment') .
            form::hidden(['remove'], 1) .
            dcCore()->formNonce() . '</div></form>';
        }
    }
}
