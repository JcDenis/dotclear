...
class FlatImport extends FlatBackup
{
    ...
    public function __construct(string $file)
    {
        ...
        $this->cur_blog        = App::core()->con()->openCursor(App::core()->getPrefix() . 'blog');
        $this->cur_category    = App::core()->con()->openCursor(App::core()->getPrefix() . 'category');
        $this->cur_link        = App::core()->con()->openCursor(App::core()->getPrefix() . 'link');
        $this->cur_setting     = App::core()->con()->openCursor(App::core()->getPrefix() . 'setting');
        $this->cur_user        = App::core()->con()->openCursor(App::core()->getPrefix() . 'user');
        $this->cur_pref        = App::core()->con()->openCursor(App::core()->getPrefix() . 'pref');
        $this->cur_permissions = App::core()->con()->openCursor(App::core()->getPrefix() . 'permissions');
        $this->cur_post        = App::core()->con()->openCursor(App::core()->getPrefix() . 'post');
        $this->cur_meta        = App::core()->con()->openCursor(App::core()->getPrefix() . 'meta');
        $this->cur_media       = App::core()->con()->openCursor(App::core()->getPrefix() . 'media');
        $this->cur_post_media  = App::core()->con()->openCursor(App::core()->getPrefix() . 'post_media');
        $this->cur_log         = App::core()->con()->openCursor(App::core()->getPrefix() . 'log');
        $this->cur_ping        = App::core()->con()->openCursor(App::core()->getPrefix() . 'ping');
        $this->cur_comment     = App::core()->con()->openCursor(App::core()->getPrefix() . 'comment');
        $this->cur_spamrule    = App::core()->con()->openCursor(App::core()->getPrefix() . 'spamrule');
        // $this->cur_version     = App::core()->con()->openCursor(App::core()->getPrefix() . 'version');
    ...
    public function importSingle(): void
    {
        ...
        $this->stack['cat_id']     = App::core()->con()->select('SELECT MAX(cat_id) FROM ' . App::core()->getPrefix() . 'category')->integer()    + 1;
        $this->stack['link_id']    = App::core()->con()->select('SELECT MAX(link_id) FROM ' . App::core()->getPrefix() . 'link')->integer()       + 1;
        $this->stack['post_id']    = App::core()->con()->select('SELECT MAX(post_id) FROM ' . App::core()->getPrefix() . 'post')->integer()       + 1;
        $this->stack['media_id']   = App::core()->con()->select('SELECT MAX(media_id) FROM ' . App::core()->getPrefix() . 'media')->integer()     + 1;
        $this->stack['comment_id'] = App::core()->con()->select('SELECT MAX(comment_id) FROM ' . App::core()->getPrefix() . 'comment')->integer() + 1;
        $this->stack['log_id']     = App::core()->con()->select('SELECT MAX(log_id) FROM ' . App::core()->getPrefix() . 'log')->integer()         + 1;

        $rs = App::core()->con()->select(
            'SELECT MAX(cat_rgt) AS cat_rgt FROM ' . App::core()->getPrefix() . 'category ' .
    ...
    public function importFull(): void
    {
        ...
        App::core()->con()->begin();
        App::core()->con()->execute('DELETE FROM ' . App::core()->getPrefix() . 'blog');
        App::core()->con()->execute('DELETE FROM ' . App::core()->getPrefix() . 'media');
        App::core()->con()->execute('DELETE FROM ' . App::core()->getPrefix() . 'spamrule');
        App::core()->con()->execute('DELETE FROM ' . App::core()->getPrefix() . 'setting');
        App::core()->con()->execute('DELETE FROM ' . App::core()->getPrefix() . 'log');
    ...
    private function insertMediaSingle(FlatBackupItem $media): void
    {
        ...
        $media->set('media_path', App::core()->blog()->settings('system')->getSetting('public_path'));
