...
class ImportFeed extends Module
{
    ...
    public function process(string $do): void
    {
        ...
        // Check feed URL
        if (App::core()->blog()->settings('system')->getSetting('import_feed_url_control')) {
            // Get IP from URL
            ...
            if (App::core()->blog()->settings('system')->getSetting('import_feed_no_private_ip')) {
            ...
            if (App::core()->blog()->settings('system')->getSetting('import_feed_ip_regexp') != '') {
                if (!preg_match(App::core()->blog()->settings('system')->getSetting('import_feed_ip_regexp'), $ip)) {
                    throw new ModuleException(__('Cannot retrieve feed URL.'));
                }
            }
            // Port control (white list regexp)
            if (App::core()->blog()->settings('system')->getSetting('import_feed_port_regexp') != '' && isset($bits['port'])) {
                if (!preg_match(App::core()->blog()->settings('system')->getSetting('import_feed_port_regexp'), (string) $bits['port'])) {
                    throw new ModuleException(__('Cannot retrieve feed URL.'));
                }
            }
        }
