...
class Prepend extends ModulePrepend
{
    ...
    public function installModule(): ?bool
    {
        ...
        if (0 == App::core()->blog()->posts()->countPosts(param: $param)
            && null == App::core()->blog()->settings('pages')->getSetting('firstpage')
        ) {
            App::core()->blog()->settings('pages')->putSetting('firstpage', true, 'boolean');
            ...
            $cur->setField('post_lang', App::core()->blog()->settings('system')->getSetting('lang'));
            ...
            // Magic tweak :)
            $old_url_format = App::core()->blog()->settings('system')->getSetting('post_url_format');
            App::core()->blog()->settings('system')->setSetting('post_url_format', '{t}');

            App::core()->blog()->posts()->createPost(cursor: $cur);

            App::core()->blog()->settings('system')->setSetting('post_url_format', $old_url_format);
        }
