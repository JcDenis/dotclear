...
class Page extends AbstractPage
{
    ...
    protected function getPagePrepend(): ?bool
    {
        ...
        try {
            $rs = $this->core->blog->getPosts(['post_type' => 'page']);
            while ($rs->fetch()) {
        ...
        try {
            $rs                         = $this->core->meta->getMetadata(['meta_type' => 'tag']);
        ...
        if ($this->core->blog->settings->system->static_home) {
        ...
        $this->core->behaviors->call('adminSimpleMenuAddType', $this->sm_items);

        $this->sm_items['special'] = new ArrayObject([__('User defined'), false]);

        # Lecture menu existant
        $menu = $this->core->blog->settings->system->get('simpleMenu');
        if (is_array($menu)) {
            $this->sm_menu = $menu;
        }
        ...
        if (!empty($_POST['saveconfig'])) {
            try {
                ...
                $this->core->blog->settings->system->put('simpleMenu_active', $menu_active, 'boolean');
            ...
            if ($this->sm_step) {
                ...
                switch ($this->sm_step) {
                    ...
                    case 3:
                        ...
                        $this->sm_item_url          = html::stripHostURL($this->core->blog->url);
                        switch ($this->sm_item_type) {
                            case 'home':
                                $this->sm_item_label = __('Home');
                                $this->sm_item_descr = $this->core->blog->settings->system->static_home ? __('Home page') : __('Recent posts');

                                break;
                            case 'posts':
                                $this->sm_item_label = __('Posts');
                                $this->sm_item_descr = __('Recent posts');
                                $this->sm_item_url .= $this->core->url->getURLFor('posts');
                            ...
                            case 'lang':
                                ...
                                $this->sm_item_url .= $this->core->url->getURLFor('lang', $this->sm_item_select);
                            ...
                            case 'category':
                                ...
                                $this->sm_item_url .= $this->core->url->getURLFor('category', $this->sm_item_select);
                            ...
                            case 'archive':
                                ...
                                if ($this->sm_item_select == '-') {
                                    ...
                                    $this->sm_item_url .= $this->core->url->getURLFor('archive');
                                } else {
                                    $this->sm_item_label = $this->sm_item_select_label;
                                    $this->sm_item_descr = sprintf(__('Posts from %s'), $this->sm_item_select_label);
                                    $this->sm_item_url .= $this->core->url->getURLFor('archive', substr($this->sm_item_select, 0, 4) . '/' . substr($this->sm_item_select, -2));
                                }
                            ...
                            case 'tags':
                                ...
                                if ($this->sm_item_select == '-') {
                                    ...
                                    $this->sm_item_descr = '';
                                    $this->sm_item_url .= $this->core->url->getURLFor('tags');
                                } else {
                                    $this->sm_item_label = $this->sm_item_select_label;
                                    $this->sm_item_descr = sprintf(__('Recent posts for %s tag'), $this->sm_item_select_label);
                                    $this->sm_item_url .= $this->core->url->getURLFor('tag', $this->sm_item_select);
                                }
                            ...
                            default:
                                ...
                                $this->core->behaviors->call('adminSimpleMenuBeforeEdit', $this->sm_item_type, $this->sm_item_select,
                    ...
                    case 4:
                        ...
                        try {
                            if (($this->sm_item_label != '') && ($this->sm_item_url != '')) {
                                ...
                                $this->core->blog->settings->system->put('simpleMenu', $this->sm_menu);
                                $this->core->blog->triggerBlog();
        ...
        if ($this->sm_step) {
            ...
            $this->setPageBreadcrumb(
                [
                    ...
                    __('Simple menu')                         => $this->core->adminurl->get('admin.plugin.SimpleMenu'),
                    __('Add item')                            => '',
                    $step_label                               => ''
                ],
                ['hl_pos' => -2]
    ...
    protected function getPageContent(): void
    {
        ...
        if ($this->sm_step) {
            ...
            switch ($this->sm_step) {
                case 1:
                    ...
                    echo '<form id="additem" action="' . $this->core->adminurl->get('admin.plugin.SimpleMenu') . '&amp;add=2" method="post">';
                    ...
                    echo '<p>' . $this->core->formNonce() . '<input type="submit" name="appendaction" value="' . __('Continue...') . '" />' . '</p>';
                    echo '</fieldset>';
                    echo '</form>';
                ...
                case 2:
                    if ($this->sm_items[$this->sm_item_type][1]) {
                        // Choix à faire
                        echo '<form id="additem" action="' . $this->core->adminurl->get('admin.plugin.SimpleMenu') . '&amp;add=3" method="post">';
                        ...
                        switch ($this->sm_item_type) {
                            ...
                            default:
                                ...
                                $this->core->behaviors->call('adminSimpleMenuSelect', $this->sm_item_type, 'item_select');
                        }
                ...
                case 3:
                    ...
                    echo '<form id="additem" action="' . $this->core->adminurl->get('admin.plugin.SimpleMenu') . '&amp;add=4" method="post">';
                    ...
                    form::field('item_label', 20, 255, [
                        ...
                        'extra_html' => 'required placeholder="' . __('Label') . '" lang="' . $this->core->auth->getInfo('user_lang') . '" spellcheck="true"'
                    ]) .
                        '</p>';
                    ...
                    echo '<p>' . $this->core->formNonce() . '<input type="submit" name="appendaction" value="' . __('Add this item') . '" /></p>';
                    echo '</fieldset>';
                    echo '</form>';
        ...
        if (!$this->sm_step) {
            echo '<form id="settings" action="' . $this->core->adminurl->get('admin.plugin.SimpleMenu') . '" method="post">' .
            '<p>' . form::checkbox('active', 1, (boolean) $this->core->blog->settings->system->simpleMenu_active) .
            '<label class="classic" for="active">' . __('Enable simple menu for this blog') . '</label>' . '</p>' .
            '<p>' . $this->core->formNonce() . '<input type="submit" name="saveconfig" value="' . __('Save configuration') . '" />' .
        ...
        // Liste des items
        if (!$this->sm_step) {
            echo '<form id="menuitemsappend" action="' . $this->core->adminurl->get('admin.plugin.SimpleMenu') . '&amp;add=1" method="post">';
            echo '<p class="top-add">' . $this->core->formNonce() . '<input class="button add" type="submit" name="appendaction" value="' . __('Add an item') . '" /></p>';
            echo '</form>';
        }

        if (count($this->sm_menu)) {
            if (!$this->sm_step) {
                echo '<form id="menuitems" action="' . $this->core->adminurl->get('admin.plugin.SimpleMenu') . '" method="post">';
            }
            // Entête table
            echo
            ...
            if (!$this->sm_step) {
                ...
                echo '<p class="col">' . form::hidden('im_order', '') . $this->core->formNonce();
