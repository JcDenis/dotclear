...
class Antispam
{
    ...
    public function purgeOldSpam(): void
    {
        ...
        $dateLastPurge = App::core()->blog()->settings('antispam')->getSetting('antispam_date_last_purge');
        if (null === $dateLastPurge) {
            $init = true;
            App::core()->blog()->settings('antispam')->putSetting('antispam_date_last_purge', $defaultDateLastPurge, 'integer', 'Antispam Date Last Purge (unix timestamp)', true, false);
        ...
        if (null === $moderationTTL) {
            App::core()->blog()->settings('antispam')->putSetting('antispam_moderation_ttl', $defaultModerationTTL, 'integer', 'Antispam Moderation TTL (days)', true, false);
        ...
        if (86400 < ($defaultDateLastPurge - $dateLastPurge)) {
            ...
            if (!$init) {
                App::core()->blog()->settings('antispam')->putSetting('antispam_date_last_purge', $defaultDateLastPurge, null, null, true, false);
            }
    ...
    public function commentsActionsPage(Action $ap): void
    {
        ...
        if (null !== App::core()->blog()->settings('antispam')->getSetting('antispam_filters')) {
            $filters_opt = App::core()->blog()->settings('antispam')->getSetting('antispam_filters');
