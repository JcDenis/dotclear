  ...
  if (ul.attr('id')) {
    ...
    $list.each(function (i) {
      ...
      if (i == 0) {
        ...
        $this.find('input.upWidget').prop('src', '?df=images/disabled_up.png');
      } else {
        $this.find('input.upWidget').prop('disabled', false);
        $this.find('input.upWidget').prop('src', '?df=images/up.png');
      }
      if (i == $list.length - 1) {
        $this.find('input.downWidget').prop('disabled', true);
        $this.find('input.downWidget').prop('src', '?df=images/disabled_down.png');
      } else {
        $this.find('input.downWidget').prop('disabled', false);
        $this.find('input.downWidget').prop('src', '?df=images/down.png');
      }
    });
  }
};
