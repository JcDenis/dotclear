...
class BlogPref extends Page
{
    ...
    protected function getPageContent(): void
    {
        ...
        # Language codes
        $lang_combo = dotclear()->combos->getAdminLangsCombo();
        ...
        if (!empty($_GET['add'])) {
            dotclear()->notices->success(__('Blog has been successfully created.'));
        }

        if (!empty($_GET['upd'])) {
            dotclear()->notices->success(__('Blog has been successfully updated.'));
        }
            ...
            foreach ($blog_users as $k => $v) {
                if (count($v['p']) > 0) {
                    ...
                    foreach ($post_type as $type => $pt_info) {
                        ...
                        echo '<li>' . sprintf(__('%1$s: %2$s'), __($pt_info['label']), dotclear()->blog()->posts()->getPosts($params, true)->f(0)) . '</li>';
                    }
                    echo
                        '</ul>';
