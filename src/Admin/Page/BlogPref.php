...
class BlogPref extends Page
{
    public function __construct(Core $core)
    {
        ...
        $standalone = !defined('EDIT_BLOG_MODE');
        ...
        if ($standalone) {
            ...
            try {
                ...
                $blog_settings = new Settings($core, $blog_id);
            ...
            try {
                ...
                if ($cur->blog_id != null && $cur->blog_id != $blog_id) {
                    if ($blog_id == $core->blog->id) {
                        ...
                        $blog_settings = new Settings($core, $cur->blog_id);
                    }
                ...
                static::addSuccessNotice(__('Blog has been successfully updated.'));
        ...
        if ($blog_id) {
            if (!empty($_GET['add'])) {
                static::success(__('Blog has been successfully created.'));
            }

            if (!empty($_GET['upd'])) {
                static::success(__('Blog has been successfully updated.'));
            }
