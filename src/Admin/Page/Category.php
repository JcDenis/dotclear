...
class Category extends Page
{
    ...
    protected function getPagePrepend(): ?bool
    {
        ...
        if (!empty($_REQUEST['id'])) {
            ...
            $parents    = core()->blog->getCategoryParents($this->cat_id);
            $rs         = core()->blog->getCategoryParent($this->cat_id);
            $this->cat_parent = $rs->isEmpty() ? 0 : (int) $rs->cat_id;
            unset($rs);

            # Allowed parents list
            $children        = core()->blog->getCategories(['start' => $this->cat_id]);
            ...
            $rs = core()->blog->getCategoryFirstChildren($this->cat_parent);
            while ($rs->fetch()) {
        ...
        if ($this->cat_id && isset($_POST['cat_parent'])) {
            ...
            if ($this->cat_parent != $new_parent) {
                try {
                    core()->blog->setCategoryParent($this->cat_id, $new_parent);
                    core()->notices->addSuccessNotice(__('The category has been successfully moved'));
        ...
        if ($this->cat_id && isset($_POST['cat_sibling'])) {
            try {
                core()->blog->setCategoryPosition($this->cat_id, (integer) $_POST['cat_sibling'], $_POST['cat_move']);
                core()->notices->addSuccessNotice(__('The category has been successfully moved'));
                core()->adminurl->redirect('admin.categories');
            } catch (Exception $e) {
                core()->error($e->getMessage());
        ...
        if (isset($_POST['cat_title'])) {
            ...
            try {
                ...
                if ($this->cat_id) {
                    ...
                    core()->behaviors->call('adminBeforeCategoryUpdate', $cur, $this->cat_id);

                    core()->blog->updCategory((int) $_POST['id'], $cur);

                    # --BEHAVIOR-- adminAfterCategoryUpdate
                    core()->behaviors->call('adminAfterCategoryUpdate', $cur, $this->cat_id);

                    core()->notices->addSuccessNotice(__('The category has been successfully updated.'));
                ...
                # Create category
                    ...
                    core()->notices->addSuccessNotice(sprintf(__('The category "%s" has been successfully created.'),
    ...
    {
        if (!empty($_GET['upd'])) {
            core()->notices->success(__('Category has been successfully updated.'));
        }

        $blog_settings = new Settings(core()->blog->id);
        ...
        if ($this->cat_id) {
            ...
            '<p><input type="submit" accesskey="s" value="' . __('Save') . '" />' .
            Form::hidden(['id'], $this->cat_id) . core()->formNonce() . '</p>' .
                '</form>' .
                '</div>';
